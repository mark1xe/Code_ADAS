#!/usr/bin/env python3
"""
cv_lane_driver.py — Tự lái OpenCV + YOLO (Night Vision & Speed Control)
=====================================================================
1. NIGHT VISION: Tăng sáng phần mềm (x2.0), hạ V_MIN=60 để nhìn trong góc tối.
2. SPEED CONTROL: Giảm DRIVE_SPEED=9 để tương thích với FF_BASE=100 của ESP32.
...
# ── Camera (Tối ưu cho góc tối / gầm giường) ──────────────────────────────
CAM_W = 320
CAM_H = 240
CAM_FIXED_EXPOSURE = 30000  # Kéo lên lại 60000 để mở mắt to hơn thu sáng
CAM_ANALOGUE_GAIN  = 4.0    # Kéo max nhạy sáng phần cứng

SW_BRIGHTNESS = 1.0         # Bơm ánh sáng phần mềm lên gấp đôi

# ── Điều khiển (Đã hạ ga cho êm) ──────────────────────────────────────────
DRIVE_SPEED  = 9            # Giảm từ 12 xuống 9 để xe bớt giật
SLOW_SPEED   = 5            # Giảm tốc độ đi chậm
SMOOTH_ALPHA = 0.75   
DEADBAND     = 0.04   
CAM_BIAS     = 0.0    
...
HSV_S_MIN, HSV_S_MAX = 0,   85  # Chấp nhận vạch ám hồng/tím
HSV_V_MIN, HSV_V_MAX = 100,  255  # Hạ kịch kim xuống 60 để bắt được vạch trong bóng tối
...
ZEBRA_SPEED         = 5    # Nếu gặp vạch đi bộ, dừng hẳn lại hoặc đi cực chậm (tùy bạn chỉnh)
...
def main():
    ...
    if not args.no_camera:
        ...
        # Bắt buộc bật Auto AWB để khử bớt độ ám hồng trong bóng tối
        _controls["AwbEnable"] = True
        print("Đã BẬT khóa Auto AWB (Camera tự khử ám màu)")
    ...
    print("═" * 50)
    print("  OPTIMIZED CV LANE DRIVER (NIGHT VISION & SLOW SPEED)  ")
    print("═" * 50)
    ...
    try:
        while True:
            ...
            # Giữ xe chạy chậm (SLOW_SPEED) khi bắt đầu mất vạch, mất quá 15 frame mới dừng hẳn
