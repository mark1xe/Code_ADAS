...
static void pin_to_core(int core_id) {
    ...
    if (rc != 0)
        fprintf(stderr, "[AFFINITY] Không ghim được core %d (err=%d)\n", core_id, rc);
    else
        fprintf(stdout, "[AFFINITY] Thread ghim vào Core %d OK\n", core_id);
}
...
constexpr float YOLO_CONF_THR   = 0.45f;   // confidence threshold
constexpr float YOLO_NMS_THR    = 0.45f;   // NMS IoU threshold
constexpr int   YOLO_NUM_CLASS  = 6;
constexpr int   YOLO_STRIDE     = 10;       // chạy YOLO cách 10 frame (tiết kiệm CPU)
const char*     YOLO_MODEL_PATH = "/home/leanhquan/export/traffic_detector.onnx";

// Class names (phải khớp thứ tự train_yolo.ipynb)
...
void init_clahe() { g_clahe = createCLAHE(3.0, Size(2, 2)); }
...
int main() {
    ...
    cout << "==============================================\n"
         ...
         << "  Core 0: main/scenario  Core 1: capture     \n"
         << "  Core 2: vision/YOLO    Core 3: uart         \n"
         << "==============================================\n"
