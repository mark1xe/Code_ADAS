...
# Nhận biết đường zebra bằng cách đếm số "cột" vạch trắng dọc trong ROI.
# Zebra có ≥ ZEBRA_MIN_STRIPES vạch đứng song song cách đều nhau.
ZEBRA_MIN_STRIPES   = 3      # số vạch dọc tối thiểu để coi là zebra
ZEBRA_SPACING_MAX   = 55     # khoảng cách tối đa (px) giữa 2 vạch liên tiếp
ZEBRA_SPACING_MIN   = 8      # khoảng cách tối thiểu (px) — tránh nhầm 1 vạch dày
ZEBRA_SLOPE_MAX     = 0.25   # vạch zebra gần thẳng đứng: |slope| < 0.25
ZEBRA_HOLD_FRAMES   = 18     # sau khi phát hiện, giữ trạng thái ZEBRA thêm N frame
                              # (tránh dao động khi xe đang đi qua)
ZEBRA_SPEED         = 7      # tốc độ giảm khi vào zebra (0 = dừng hẳn)
ZEBRA_STEER_LOCK    = True   # True = giữ thẳng (steer=0) khi qua zebra
...
def detect_zebra_crossing(frame_rgb: np.ndarray) -> bool:
    """
    Phát hiện đường zebra crossing trong frame.

    Thuật toán:
      1. Lấy dải ROI dưới (50%–85% chiều cao) — vùng xe sắp đến.
      2. Ngưỡng trắng (Otsu) → tìm contour.
      3. Lọc contour theo tỉ lệ: cao/rộng > 1.5 (vạch đứng) và diện tích hợp lý.
      4. Nếu có ≥ ZEBRA_MIN_STRIPES contour xếp cạnh nhau (khoảng cách đều) → ZEBRA.

    Trả về True nếu phát hiện zebra.
    """
    ...
    for cnt in contours:
        ...
        area = w * h
        # Lọc: vạch đứng (cao hơn rộng), không quá nhỏ, không quá lớn
        if h < roi_h * 0.25:           # quá ngắn → không phải vạch zebra
            continue
        if w == 0 or h / w < 1.2:     # quá ngang → không phải vạch đứng
            continue
        if area < 150 or area > W * roi_h * 0.25:   # quá nhỏ hoặc quá lớn
            continue
        cx_stripe = x + w / 2.0
        stripe_centers_x.append(cx_stripe)

    if len(stripe_centers_x) < ZEBRA_MIN_STRIPES:
        return False

    # Kiểm tra khoảng cách đều: sắp xếp theo x, đo spacing
    stripe_centers_x.sort()
    spacings = [stripe_centers_x[i+1] - stripe_centers_x[i]
                for i in range(len(stripe_centers_x) - 1)]

    # Đủ spacing hợp lệ → zebra
    valid_spacings = [s for s in spacings
                      if ZEBRA_SPACING_MIN <= s <= ZEBRA_SPACING_MAX]
    return len(valid_spacings) >= ZEBRA_MIN_STRIPES - 1
...
def main():
    ...
    try:
        while True:
            ...
            # ── Zebra override: giảm tốc + giữ thẳng ─────────────────
            if in_zebra:
                spd = ZEBRA_SPEED
                if ZEBRA_STEER_LOCK:
                    steer_val = 0
                    smooth_steer = 0.0   # reset EMA để không tích lũy sai lệch
