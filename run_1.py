...
# Dùng column-histogram trên dải TRUNG TÂM (tránh nhầm lane cong ở rìa).
# Zebra tạo ra nhiều CỘT trắng đứng đều đặn xen kẽ cột đen.
ZEBRA_MIN_STRIPES   = 4      # cần ≥ 4 vạch trắng → tránh nhầm 2–3 vạch lane
ZEBRA_HOLD_FRAMES   = 22     # giữ trạng thái ZEBRA N frame sau khi mất signal
ZEBRA_SPEED         = 8      # tốc độ khi qua zebra
ZEBRA_STEER_LOCK    = False  # KHÔNG lock steer=0 — xe vẫn bám lane qua zebra
                              # (lock=True gây giật vì EMA bị reset đột ngột)
...
_zebra_hold_cnt    = 0     # đếm ngược frame còn trong trạng thái ZEBRA
...
def detect_zebra_crossing(frame_rgb: np.ndarray) -> bool:
    """
    Phát hiện zebra crossing bằng COLUMN HISTOGRAM trên vùng TRUNG TÂM.

    Tại sao trung tâm?
    - Lane cong nằm ở RÌA trái/phải → histogram cột ở rìa có peak giả.
    - Zebra trải NGANG toàn bộ làn kể cả trung tâm → vẫn thấy ở vùng giữa.
    - Lấy 40% giữa (30%–70% chiều ngang) + dải dưới (55%–80% chiều cao).

    Các bước:
      1. Crop vùng trung tâm-dưới.
      2. Otsu threshold → ảnh nhị phân.
      3. Tính % pixel trắng mỗi cột → col_white[].
      4. Đếm số "đoạn sáng" liên tiếp (run of bright columns).
      5. Kiểm tra spacing các đoạn có đều không (CV < 0.45).
      6. ≥ ZEBRA_MIN_STRIPES đoạn đều → ZEBRA.
    """
    H, W = frame_rgb.shape[:2]

    # Vùng trung tâm: tránh rìa có lane cong
    ...
    roi_h = y1 - y0
    # Tỉ lệ pixel trắng mỗi cột
    col_white = thresh.sum(axis=0) / (roi_h * 255.0)  # shape (x1-x0,)

    # Ngưỡng: cột "sáng" nếu >35% pixel trắng
    BRIGHT_THRESH = 0.35
    is_bright = col_white > BRIGHT_THRESH

    # Đếm số đoạn sáng liên tiếp
    bright_runs = []
    in_run = False
    run_start = 0
    for i, b in enumerate(is_bright):
        if b and not in_run:
            ...
            in_run = False; bright_runs.append((run_start, i - 1))
    if in_run:
        bright_runs.append((run_start, len(is_bright) - 1))

    if len(bright_runs) < ZEBRA_MIN_STRIPES:
        return False

    # Kiểm tra spacing đều (zebra) vs không đều (nhiễu/lane)
    centers = [(s + e) / 2.0 for s, e in bright_runs]
    spacings = [centers[i+1] - centers[i] for i in range(len(centers) - 1)]
    if not spacings:
        return False

    mean_sp = float(np.mean(spacings))
    if mean_sp < 5 or mean_sp > 50:   # spacing hợp lý 5–50px
        return False

    cv_sp = float(np.std(spacings)) / mean_sp   # coefficient of variation
    return cv_sp < 0.45   # đều → zebra
...
def detect_lane_offset(frame_rgb: np.ndarray, debug: bool = False):
    ...
    if debug and dbg is not None:
        ...
        # Hiển thị trạng thái zebra trên debug frame
...
def main():
    ...
    try:
        while True:
            ...
            if is_zebra:
                _zebra_hold_cnt = ZEBRA_HOLD_FRAMES   # reset bộ đếm hold
            ...
            # ── Zebra override: chỉ giảm tốc, KHÔNG reset steer/EMA ──────
            if in_zebra:
                spd = ZEBRA_SPEED
                # ZEBRA_STEER_LOCK=False → xe vẫn bám lane bình thường,
                # chỉ chậm lại khi qua vạch kẻ đường
            ...
            print(f"\r[{state_tag}] {lane_tag}  fps={fps:4.1f}  "
                  ...
                  f"noln={no_lane_cnt:3d}  zbr={_zebra_hold_cnt:2d}",
                  end="", flush=True)
