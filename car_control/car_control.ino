...
void computePID() {
  ...
  if (now - prevTime < 20) return;  // 20ms = 50Hz — mượt hơn 50ms (20Hz)
                                     // RPi gửi 10Hz nhưng PID nên chạy nhanh hơn
...
// --- SERVO với SMOOTHING ---
// =============================================
// Thay vì nhảy thẳng đến desiredServoAngle, di chuyển từng bước nhỏ
// → servo không giật, xe không nất cục khi steer thay đổi nhanh
const int SERVO_STEP = 3;   // tối đa 3 độ/lần gọi updateServo() (~150Hz)
                             // = ~10 độ/20ms → đủ nhanh mà không giật

void updateServo() {
  if (desiredServoAngle == currentServoAngle) return;

  int diff = desiredServoAngle - currentServoAngle;
  if (abs(diff) <= SERVO_STEP) {
    currentServoAngle = desiredServoAngle;
  } else {
    currentServoAngle += (diff > 0) ? SERVO_STEP : -SERVO_STEP;
  }
...
void carForward(int spdL, int spdR) {
  // Chỉ reset encoder/integral khi xe đang đứng yên (tránh reset 10 lần/giây từ RPi)
  if (targetSpeedLeft <= 0 && targetSpeedRight <= 0) {
