...
def detect_zebra_crossing(frame_rgb: np.ndarray) -> bool:
    ...
    Chống nhiễu ánh sáng tốt hơn hẳn việc đếm sọc ngựa vằn.
    """
    H, W = frame_rgb.shape[:2]
    
    # Chỉ quét ở nửa dưới màn hình để bắt vạch gần đầu xe nhất
    roi_y = int(H * 0.4) 
    
    gray = cv2.cvtColor(frame_rgb, cv2.COLOR_RGB2GRAY)
    blur = cv2.GaussianBlur(gray, (7, 7), 0)
    
    # Ép ngưỡng Threshold tĩnh cho vạch ngang để đảm bảo bắt nét
    _, thresh = cv2.threshold(blur, 160, 255, cv2.THRESH_BINARY)
    edges = cv2.Canny(thresh, 80, 150)
    
    # Tạo mask chỉ giữ lại viền ở nửa dưới
    roi_edges = np.zeros_like(edges)
    roi_edges[roi_y:H, :] = edges[roi_y:H, :]

    # Tìm các đường thẳng dài (Đặc trưng của vạch ngang)
    lines = cv2.HoughLinesP(
        roi_edges,
        rho=1, theta=np.pi / 180,
        threshold=40,
        minLineLength=int(W * 0.25),  # Phải dài ít nhất 25% chiều rộng khung hình
        maxLineGap=20
    )
    ...
    if lines is not None:
        for line in lines:
            ...
            # Tính độ dốc (slope). Đường ngang sẽ có hệ số góc tiệm cận 0
...
def main():
    ...
    try:
        while True:
            ...
            # ── Lane detection ─────────────────────────────────────────
            if in_zebra:
                # Ép xe đi thẳng qua vùng vạch đi bộ
                offset  = 0.0
                dbg_img = None
            else:
